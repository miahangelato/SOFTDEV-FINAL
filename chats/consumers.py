...
class MychatApp(AsyncWebsocketConsumer):
    ...
    async def connect(self):
        print(f"WebSocket connected. User: {self.scope['user']}, Authenticated: {self.scope['user'].is_authenticated}")
        await self.accept()
        await self.channel_layer.group_add(f"mychat_app_{self.scope['user'].username}", self.channel_name)
